...
sub jump {
    ...
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_jump', \%args);
    return 1 if $code eq 200;
    return 0 if $code eq 450;
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub jump_back {
    ...
    my %args = (DB => $self->{db}, CUR => $self->{cursor});
    $args{key} = $key if defined $key;
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_jump_back', \%args);
    return 1 if $code eq 200;
    return 0 if $code eq 450;
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub step {
    ...
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_step', \%args);
    return 1 if $code eq '200';
    return 0 if $code eq '450';
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub step_back {
    my ($self, ) = @_;
    my %args = (CUR => $self->{cursor});
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_step_back', \%args);
    return 1 if $code eq '200';
    return 0 if $code eq '450';
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub set_value {
    ...
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_set_value', \%args);
    return 1 if $code eq '200';
    return 0 if $code eq '450';
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub remove {
    my ($self,) = @_;
    my %args = (CUR => $self->{cursor});
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_remove', \%args);
    return 1 if $code eq '200';
    return 0 if $code eq '450';
    die Cache::KyotoTycoon::_errmsg($code, $msg, $raw_body);
}
...
sub get_key {
    ...
    my %args = (CUR => $self->{cursor});
    $args{step} = '' if defined $step;
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_get_key', \%args);
...
sub get_value {
    ...
    my %args = (CUR => $self->{cursor});
    $args{step} = '' if defined $step;
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_get_value', \%args);
...
sub get {
    ...
    my %args = (CUR => $self->{cursor});
    $args{step} = '' if defined $step;
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_get', \%args);
...
sub delete {
    my ($self, ) = @_;
    my %args = (CUR => $self->{cursor});
    my ($code, $body, $msg, $headers, $raw_body) = $self->{client}->call('cur_delete', \%args);
